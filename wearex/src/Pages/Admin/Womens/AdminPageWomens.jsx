...
const AdminPageWomens = () => {

    const [data,setData] = useState([])
    
    const  getWomensProduct = ()=>{
        axios.get(`${BaseURL}/womensProduct`)
    ...
    const handleDelete = (id)=>{
        axios({
            method : 'delete',
            url : `${BaseURL}/womensProduct/${id}`,
        }).then(()=>getWomensProduct()).catch((err)=>console.log(err))
    }
  ...
  return (
    <Box>
        <Link to='/adminpagewomensaddproduct' ><Button ml={'90%'} my={4} bg={'green.300'} >Add Product</Button></Link>
        <SimpleGrid pl={2} gridTemplateColumns={{base:'repeat(2,1fr)' , sm:'repeat(4,1fr)'}} gap={{base:'0px',sm:'15px'}} >
                    {
                        data?.map((el)=>(
                            <Box
                                ...
                                <Center mt={3}>
                                    <HStack>
                                        <Link to={`/adminpagewomenseditproduct/${el.id}`} ><Button color={'white'} height={'30px'} bg={'green.600'} >Edit</Button></Link>
...
export default AdminPageWomens
